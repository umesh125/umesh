...
/**
 *  @file
 *  Create the 'S3' tab for the WYSIWYG plugins.
 */

// (function ($) {
//   namespace('Drupal.media.browser.plugin');
//
//   Drupal.media.browser.plugin.media_s3 = function(mediaBrowser, options) {
//     return {
//       init: function() {
//         tabset = mediaBrowser.getTabset();
//         tabset.tabs('add', '#media_s3', 'S3');
//         mediaBrowser.listen('tabs.show', function (e, id) {
//           if (id == 'media_s3') {
//             // We only need to set this once.
//             // We probably could set it upon load.
//             if (mediaBrowser.getActivePanel().html() == '') {
//               mediaBrowser.getActivePanel().html(options.media_s3);
//             }
//           }
//         });
//       }
...
//   //@todo: needs a review.
//   Drupal.media.browser.register('media_s3', Drupal.media.browser.plugin.media_s3, {});
// })(jQuery);

(function ($) {
  namespace('media.browser.plugin');

  Drupal.media.browser.plugin.s3_library = function(mediaBrowser, options) {

    return {
      mediaFiles: [],
      init: function() {
        tabset = mediaBrowser.getTabset();
        tabset.tabs('add', '#s3_library', 'S3');
        var that = this;
        mediaBrowser.listen('tabs.show', function (e, id) {
          if (id == 's3_library') {
  ...
  Drupal.media.browser.register('s3_library', Drupal.media.browser.plugin.s3_library);
})(jQuery);
